...
1. **생성물 격리**: `src/api/` import 는 훅·매핑 계층에서만. 컴포넌트가 DTO 를 직접 쓰면 위반 — 뷰 모델(`src/types/`)과 `src/lib/api-mapping.ts` 를 거쳐야 함.
...
3. **섹션 구조**: 섹션명·"필수"·"완료 조건" 하드코딩 금지. 섹션은 백엔드 `task_sections` 행이 정본(`is_required` 그대로 표시), 프론트는 `body` 만 섹션 PATCH 로 바꾼다(`version` 동봉). 예제 마커는 `MARKER_RE`/`countMarkers` 만 사용. 제안(proposal)은 `section_id`·`section_version` 으로 섹션과 잇고 diff 는 `src/lib/diff.ts` 로 프론트가 계산한다 — 섹션명으로 매칭하거나 서버 diff 문자열을 파싱하면 위반.
