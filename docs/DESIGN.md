...
> 대상 화면은 범위 1~3: **태스크 생성(유형 템플릿) · 태스크 상세(섹션 편집 · 제안 diff 수락/거부 · 미결정 답변) · 업무 보드(개발 준비됨 게이트)** + 태스크와 무관한 **구현 확인**(코드 구현 질의 채팅, 2026-09-05 추가).
> 목업은 `docs/design/` 아트보드 11장(화면 8 + 상태 시트 3). 2026-09-01 디자인 리뷰 + 범위 1 스펙 개정(완료 조건 폐기 → 세부사항·예외 조건 필수, 단어 단위 diff) 반영본, 2026-09-05 구현 확인 3장(`AskEmpty` · `AskAnswer` · `AskStates`) 추가. 2026-09-06 `AskEmpty` 예시 질문을 카테고리 탭 6개 + 문항으로 갱신.
...
| diff 위치                             | **문서의 대상 섹션 위에 diff 와 [거부][수락]** 을 렌더링한다(spec 문구 그대로). diff 는 **단어 단위(LCS, 프론트 계산)** — 삭제는 취소선 + `error-wash`, 추가는 `success-wash` + 500. 교체 단위는 섹션이지만 표시는 단어 단위다. Navi 패널에는 사유·대상·상태와 "문서에서 보기"만 있는 **요약 카드**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 템플릿 구조 (범위 1 개정)             | 섹션 축은 **정책(무엇) → 세부사항(어떤 값으로, `항목 — 값`) → 예외 조건(언제 제외)**. **세부사항·예외 조건 필수**, 완료 조건·인수 기준은 담당자에게 요구하지 않는다. 고정 문구는 세부사항 아래 `### 알림톡 템플릿` 코드블록(마커 유지). 섹션은 백엔드 `task_sections` 행(`name`·`is_required`)이며 프론트는 이름·필수 여부를 그대로 표시한다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 편집 본문의 새 헤딩 (코드 리뷰 후)    | 편집 중 본문에 `## ` 줄이 있으면 **저장을 차단**하고 안내 문구를 보인다(`###` 소제목과 코드 펜스 안은 허용). 섹션 식별은 이름이 아니라 **섹션 id**. 저장은 `PATCH /tasks/{id}/sections/{section_id}` 에 섹션 `version` 을 동봉하고 409 면 toast 후 편집기를 열어 둔 채 최신 내용을 다시 받는다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 취소·아카이브 태스크 (코드 리뷰 후)   | 취소는 보드에서 숨기고, 아카이브는 보드의 **'아카이브' 뷰**(필터 행 구분선 뒤 칩, 평면 카드 목록 + 상태 pill, 드래그 없음)에서만 보인다. 상세로 진입하면 **읽기 전용 배너** + 편집/분석 버튼 숨김 (리다이렉트 아님)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 태스크 취소 · 복원 (2026-09-01)       | 취소는 확인 다이얼로그(480) 후 `status=CANCELED` — 빨간 채움 없이 `button-secondary` + `error-deep` 텍스트, 토스트에 **되돌리기** 액션. 취소된 상세의 읽기 전용 배너에 **"Backlog로 복원"**. 아카이브는 kebab(상세·카드)의 **"아카이브"**(기본 variant, 되돌릴 수 있어 destructive 아님)로 확인 없이 실행, 토스트에 되돌리기. 아카이브된 상세의 배너에 **"아카이브 해제"**, 아카이브 카드 kebab 은 상세 열기·아카이브 해제만. `--destructive` 는 `error-deep` 으로 매핑(메뉴 destructive 항목 텍스트)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 구현 확인 (2026-09-05)                | 비개발자가 "지금 구현이 어떻게 되어 있나"를 채팅으로 묻는 **독립 탭**(`/ask`, 태스크와 무관). MVP 는 **서버 저장 없음** — 대화는 세션 로컬이고 후속 질문은 이전 턴을 `history[]` 로 동봉해 이어 간다. 응답은 **스트림**(thinking · tool_call · tool_result · answer · done · error)이며 **과정 블록을 접지 않고 전부 표시**하고, 근거는 답변 본문의 "근거" 목록 + 답변 아래 **메타 줄**(확인한 레포 · 커밋 · 탐색 횟수 · 소요 시간)로 보인다(사용자 결정, 참고한 코드 카드는 중복이라 폐기). 대상 코드베이스 3개(Spring Boot·Kotlin / Django·Python / React·TypeScript)는 **사용자가 고르지 않는다** — 백엔드 LLM 이 선택 가이드로 **질문마다** 고르고, 프론트는 과정 블록 첫 단계 "대상 선택"으로 보여 준다(같은 날 2차 결정 — 선택 이벤트는 없고 첫 `tool_call.arguments.repo` 로 합성, 레포는 기술명 그대로). 덮어쓰기 UI 없음(질문에 "어드민 기준으로"처럼 적으면 Navi 가 따른다), 판별이 모호하면 Navi 가 일반 답변으로 되묻는다. 기존 Navi 패널(제안 파이프라인)은 재사용하지 않되 문체 규칙은 같다(시스템 합니다체 · Navi 해요체). 화면의 코랄 채움은 전송(진행 중엔 중단) 버튼 1개. 실제 스트림 명세와 표시 규칙은 `docs/ask-stream-contract.md`. 빈 상태 예시 질문은 **카테고리 탭 6개 + 선택 카테고리 문항 2~3개**(2026-09-06 — 16개를 다 펼치면 빈 상태가 길어져 탭으로 접음, §D.4 `ask-suggestions`) |
| 범위 2 연동 (2026-09-04 섹션 모델)    | 제안은 **섹션 id · 제안 시점 섹션 version** 으로 문서와 잇는다. diff 는 **프론트가 계산**한다(`src/lib/diff.ts`: 줄 LCS → 바뀐 줄끼리 어절 LCS, 공통 글자 30% 미만이면 줄 삭제·추가) — 목록 응답에 제안 본문이 있어 리로드 후에도 미리보기가 된다. 거부는 같은 응답으로 재제안이 돌아온다. **만료 = 서버 `is_stale` · 섹션 version 차이 · accept 409** 중 하나 — 배너에 "다시 제안 받기" + "닫기"(= 서버 CLOSED, 수락·거부 없이 종결). 대기 제안이 있는 섹션은 편집 버튼을 숨긴다(편집하면 만료). 채팅은 동기(최대 60초) — busy 점 3개 + 입력창을 중단 버튼으로 교체(AbortController). 대화는 stateless 세션 로컬 — 리로드 후 대기 제안은 패널 상단 요약 카드 + 문서 블록으로 보인다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 컴포넌트                  | 스펙                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app-topbar`              | 높이 56, bg `{colors.canvas}`, 하단 1px hairline. 좌: 코랄 점 8px + "Navi" 15/600 · 구분선 · 탭("업무 보드"·"구현 확인", 활성 bg `{colors.surface-card}`, 비활성 muted + hover `surface-soft`; "분석"은 범위 3 라우트가 생기면 추가) 또는 브레드크럼 · 우: 검색(240×36, bg `surface-soft`, 구현 확인 화면에는 없음) + 화면별 액션 슬롯(코랄 1개 + secondary + kebab 36px) |
...
| 상단바 액션                                         | `button-icon` kebab("태스크 취소") + 분석 전: `button-primary` **"분석 시작"**(문서에 수락 가능한 제안 블록이 있으면 `button-secondary` 로 내려 코랄을 "수락"에 양보 — 블록이 여러 개면 "수락"도 여러 개) · 분석 중: disabled + 스피너 "분석 중…" · 분석 후: `button-secondary` "분석 다시 실행" (코랄은 제안의 "수락"에 양보). 읽기 전용이면 분석 시작만 숨기고 kebab 은 아카이브/해제용으로 남긴다 |
...
| 컴포넌트                     | 스펙                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 섹션 헤더 표시               | 대상 섹션 h2 옆 중립 pill + 코랄 점 "제안 대기"(만료 "제안 만료" 경고색 · 거부 직후 "재제안 대기" 회색 점). 섹션당 블록은 최신 대기 제안 1개. 대기 중엔 편집 버튼 숨김. 패널 카드 "문서에서 보기" 클릭 → 이 섹션으로 스크롤                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `section-proposal` (pending) | 섹션 본문 자리에 블록: 1px `{colors.primary}` · `{rounded.lg}` · overflow hidden. **헤더** bg `primary-wash`: 코랄 점 + "Navi 제안" 13/500 `primary-text` + 사유 13 body(해요체). **diff** 는 섹션 본문(16/1.7) 안에 **단어 단위 인라인**: 삭제 = bg `error-wash` + 취소선 + `error-deep`, 추가 = bg `success-wash` + 500 + `success-deep`, 변경 없는 단어는 `body` 그대로. 프론트가 현재 본문과 제안 본문으로 LCS 를 계산해 `equal/delete/insert` 런을 그린다. 통째로 새로 들어간 줄은 줄 전체가 추가 런. **푸터** hairline-soft 상단선: 좌 12 muted "수락하면 이 섹션만 교체되고 vN+1로 저장됩니다" · 우 `button-destructive-text` "거부" + `button-primary` "수락" |
| `field-proposal-card`        | (보류 — 백엔드가 `update_field` 를 없애고 제목·태그 제안은 `no_change` 답변으로 대신한다) 메타 필드 변경 제안은 문서 **상단**에 카드로: 1px `{colors.primary}` · 헤더 "메타 필드 변경 제안" `primary-text` · `필드: 이전 → 새 값` 단어 diff · 같은 액션 행. 백엔드에 메타 필드가 생기면 활성화                                                                                                                                                                                                                                                                                                                                                                        |
...
| `section-proposal-stale`     | hairline 블록 상단에 `warning-wash` 배너: `AlertTriangle` + 13/500 "문서가 변경되어 이 제안은 만료되었습니다 (vN에서 ○○이 수정됨)" + `button-ghost` "다시 제안 받기" · "닫기"(= 서버 CLOSED, 수락·거부 없이 종결). diff 는 opacity .5, 수락/거부 없음. **수락 시 낙관적 잠금 충돌도 같은 배너**, 문구는 서버 메시지. 만료 블록에서는 편집 버튼을 유지한다                                                                                                                                                                                                                                                                                                             |
...
| `task-card-menu`                            | kebab 메뉴 176px: 상세 열기 · 구분선 · 우선순위 ▸(라디오, 현재 값 12 muted) · 이동 ▸(현재 컬럼 제외) · 아카이브 · 구분선 · **태스크 취소**(destructive = `error-deep` 텍스트). 아카이브된 카드: 상세 열기 · 구분선 · 아카이브 해제. 메뉴 안 이벤트는 카드 클릭·드래그 센서로 번지지 않는다                                                                                                                                                                                                                                                                                       |
...
| 필터 행                                     | 36px 칩: 전체(활성 bg `surface-card`) · 내 태스크 · 준비됨만 · 구분선 · 아카이브(뷰 전환) · "유형 ▾"(아이콘↔라벨 대응을 여기서 노출)                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
#### D.4 구현 확인 (2026-09-05) — `AskEmpty.dc.html`(빈 상태) · `AskAnswer.dc.html`(답변 완료 + 후속 질문 진행 중) · `AskStates.dc.html`(상태 시트)

단일 컬럼(폭 760, 가운데 정렬). 상단바는 탭 "구현 확인" 활성, 검색 없음, 액션 슬롯은 `button-secondary` "새 대화"(빈 상태에서는 disabled). 대화는 위에서 아래로 쌓이고 입력창은 하단 고정. **대상 코드베이스 선택 UI 는 없다** — 백엔드 LLM 이 질문마다 고르고, 프론트는 첫 `tool_call.arguments.repo` 로 과정 블록 첫 단계 `ask-target-step` 을 합성한다. 화면의 코랄 채움은 **전송 버튼 1개**(진행 중엔 같은 자리가 중단 버튼). 스트림 이벤트 ↔ 화면 대응과 백엔드 요청 사항은 `docs/ask-stream-contract.md`.

| 컴포넌트          | 스펙                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ask-empty`       | 상단 112px 여백 뒤 가운데 정렬: 타이틀 serif 28/400 "구현 확인" · 14 body "코드를 직접 읽고 지금 구현이 어떻게 되어 있는지 답합니다. 어느 서비스의 코드인지는 Navi 가 고릅니다." · 13 muted "대화는 저장되지 않습니다. 새로 고치면 사라집니다."                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `ask-target-step` | 프론트가 **첫 `tool_call.arguments.repo` 로 합성**(`ask-step` 문법): `Layers` 16 muted + 라벨 "대상 선택" + repo 기술명 `code` 12(`fitpetmall-backend-v4` — 표시명 API 없음, 그대로) + 완료 체크. 같은 턴에서 새 repo 가 나오면 "대상 추가" 행. 고른 이유는 직전 `thinking` 행이 담당한다. 과정 헤더는 "탐색 과정 · N단계"(대상 이름 없음). 후속 질문에도 매 턴 다시 나온다                                                                                                                                                                                                                                                                                                                                                                                                           |
| `ask-suggestions` | 라벨 12/500 muted "이런 질문을 할 수 있습니다" + **카테고리 탭 행**(`category-tab` 6개, 13/500, 높이 32, padding 0 14, `{rounded.md}`: 주문·결제 · 적립금·쿠폰 · 배송·반품 · 회원·등급 · 알림·자동 작업 · 화면·노출. 활성 bg `surface-card` ink, 비활성 투명 muted + hover `surface-soft`. 첫 진입은 첫 카테고리 활성, 클릭은 전송하지 않고 아래 문항만 바꾼다, 상태는 컴포넌트 로컬 · 저장 없음) + 선택된 카테고리의 32px pill 2~3개(`suggestion-chips` 와 같은 스타일, 문장형 질문 — pill 자체는 한 줄 고정, pill 행이 760 폭에서 감싸져도 최대 2줄). pill 클릭 시 그대로 전송(카테고리명은 보내지 않으므로 문장이 혼자서 대상을 고를 수 있어야 한다). 문항 정본은 `src/lib/ask-config.ts`(16개, 2026-09-06 — 카테고리화하며 3개 → 16개, 정기배송·멤버십 문항은 기능 없음으로 제외) |
| `ask-input`       | 폭 760 · min-height 56 · bg `surface-soft` + hairline(canvas 위 단차) · `{rounded.lg}` · padding 10 10 10 16 · textarea 14(Enter 전송 · Shift+Enter 줄바꿈). 우측 **원형 32 코랄 채움**(`ArrowUp` 14, 이 화면의 유일한 코랄 채움) · 입력이 비었을 때 disabled(bg `primary-disabled` + 아이콘 muted). **진행 중**: opacity .85, 텍스트 `muted-soft` "응답이 끝나면 다시 질문할 수 있습니다", 같은 자리에 코랄 채움 **중단**(`Square` 12)                                                                                                                                                                                                                                                                                                                                               |
| `ask-user`        | 우측 정렬, bg `surface-card`, `{rounded.lg}`, padding 10 14, 14 ink, 최대 560                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `ask-navi-header` | 코랄 점 8 + "Navi" 13/500 `primary-text`. 중단 시 옆에 중립 `badge-pill` "중단됨"(`Square` 10 muted)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `ask-process`     | **항상 펼침, 접기 없음.** bg `surface-soft` · `{rounded.lg}` · padding 12 14 · gap 12. 헤더 12/500 muted "탐색 과정 · N단계" + 우측 `code` 12 경과 시간. 진행 중 헤더는 teal 점 3개(8px) + "탐색 중 · N단계"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `ask-step`        | 아이콘 16 muted(생각 `Lightbulb` · 폴더 보기 `FolderOpen` · 코드 검색 `Search` · 파일 읽기 `FileText` · 미지 도구 `Wrench`) + 라벨 13/500 body + 대상 `code` 12 muted + 우측 상태(완료 `CircleCheck` 14 `success-deep` · 진행 teal 점 3개 6px · 실패 `X` 14 `muted-soft`). `tool_result.summary` 는 아래 12 muted. **thinking 텍스트는 13 muted 1.5 로 전부 표시.** 도구 이름은 프론트 매핑(`list_dir` 폴더 보기 · `search_code` 코드 검색 · `read_file` 파일 읽기, 미지 도구는 원본), target 은 `arguments` 로 조립(`repo:path 'pattern' 12~80줄`). 실패는 summary 의 "오류" 문자열로 판정(best-effort)                                                                                                                                                                              |
| `ask-note`        | 도구 호출 사이에 온 `answer` 텍스트(중간 설명). 다음 `tool_call` 이 오면 답변 영역에서 과정 블록 안 행으로 옮긴다: `MessageSquareText` 16 muted + 13 body 1.5. 마지막 answer 만 `ask-answer` 에 남는다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `ask-answer`      | `MarkdownDoc variant="chat"`(`prose-doc prose-chat`, 마커 하이라이트 없음): 14 body 1.7, 강조 600 ink, 인라인 코드 mono 13 bg `surface-card` `{rounded.xs}` padding 1 5, 번호 목록 표시. 코드 블록은 bg `surface-dark` `{rounded.md}` + 본문 mono 13 `on-dark`(파일명 헤더는 보류 — 펜스 메타를 react-markdown 이 노출하지 않음). 스트리밍 중 커서 없음, 텍스트만 누적. 답변 끝 "근거" 목록(`레포/경로:라인 — 설명`)은 백엔드 프롬프트가 강제. 해요체                                                                                                                                                                                                                                                                                                                                 |
| `ask-meta`        | 답변 아래 한 줄 12 muted: "확인한 레포 `fitpetmall-backend-v4 @ f3818f6`(, …) · 2회 탐색 · 8.8초" — `done.repos`(commit null 이면 `@` 생략, mono) · `iterations` · `elapsed_ms`. 별도 참고한 코드 카드는 두지 않는다(답변 본문 근거 목록과 중복 — 2026-09-05 사용자 결정)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `ask-connecting`  | 첫 이벤트 전: teal 점 3개 + 13 muted "코드베이스에 연결하는 중…"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `ask-aborted`     | 받은 과정·답변은 유지. 헤더에 pill "중단됨", 과정 헤더 "탐색 과정 · N단계 · 중단", 진행 중이던 단계는 `X` 상태, 아래 12 muted "여기까지 받은 답변만 표시합니다"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `ask-error`       | D.0 `callout` error: 14/500 "답변을 받지 못했습니다" + 13 서버 `message`(합니다체). "다시 시도"(`button-ghost` 32 `error-deep`, 같은 질문을 새 턴으로 재전송하고 실패 턴은 지움)는 **503(LLM 불가·탐색 40회 한도) · 네트워크 단절 · 종결 이벤트 없는 조기 종료("응답이 중간에 끊겼습니다")에만**, 500 은 메시지만                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `ask-clarify`     | 대상을 판별하지 못해 되묻는 경우: 도구 호출 없이 `answer` 만 오므로 **과정 블록 없는 일반 답변 턴**(Navi 헤더 + 답변, 해요체). 새 이벤트 없음. 사용자가 답을 입력하면 다음 턴에서 정상 탐색                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 대화 유지         | 서버 저장 없음. "새 대화"는 확인 없이 초기화(되돌리기 없음), 리로드 시 소멸. 요청 `messages[]` 에는 **완료(done)된 턴의 질문 · 최종 답변 쌍**만 넣는다(과정 · 중단 · 오류 턴 제외, 최근 20턴)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- [ ] 구현 확인: 과정 블록은 접지 않고 전부 표시 + 답변 아래 메타 줄(참고한 코드 카드 없음), 도구 이름은 한국어 라벨(프론트 매핑), 대상 레포는 첫 tool_call 로 합성한 "대상 선택" 행(선택 칩 없음 · 기술명 그대로), 코랄 채움은 전송/중단 버튼 1개, 빈 상태 예시 질문은 카테고리 탭 6개 + 선택 카테고리 문항 2~3개(정본 `src/lib/ask-config.ts`)
