...
describe('computeGate', () => {
  it('마커가 남아 있으면 첫 실패는 markers', () => {
    const gate = computeGate({ content: '- (예: x)', analysisCount: 1, undecided: [answered] })
  ...
  it('분석 전이면 미결정도 통과로 치지 않는다', () => {
    const gate = computeGate({ content: '- 실제 값', analysisCount: 0, undecided: [] })
  ...
  it('세 조건을 모두 만족하면 통과', () => {
    const gate = computeGate({ content: '- 실제 값', analysisCount: 1, undecided: [answered] })
  ...
  it('분석이 미결정 0건을 뽑아도 통과', () => {
    expect(computeGate({ content: '- 값', analysisCount: 2, undecided: [] }).passed).toBe(true)
  })
})
